...
void PangolinDSOViewer::run()
{
	...
	pangolin::CreateWindowAndBind("Main",2*w,2*h);//create GUI window to show,here is the window name,and the window size
	...
	pangolin::CreatePanel("ui").SetBounds(0.0, 1.0, 0.0, pangolin::Attach::Pix(UI_WIDTH));//the left panel at the window left
