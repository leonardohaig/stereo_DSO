...
protected:
    int w, h,//the output image's width and height
    wOrg, hOrg, //the input image's width and height
    wUp, hUp;


    float bl;//the length of baseline,unit:m
    int upsampleUndistFactor;
	Mat33 K;
	VecX parsOrg;//the camera's internal params(5 params,separately express:fx,fy,ox,oy,0)
	bool valid;
	bool passthrough;
